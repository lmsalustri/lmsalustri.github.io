<script lang="ts">
	const officersPage = {
		title: 'Officers | WPI Neurodivergent Student Association',
		canonical: 'https://lmsalustri.github.io/wpi-ndsa/officers/',
		heading: 'Officers',
		officers: [
			{
				role: 'President',
			...
			{
				role: 'Treasurer',
				name: 'Devin Engelbrecht',
				major: 'Aerospace Engineering & Physics, Class of 2029',
				image: '/wpi-ndsa/images/engelbrecht.JPG',
...
<svelte:head>
	<title>{officersPage.title}</title>
	<link rel="canonical" href={officersPage.canonical} />
</svelte:head>

<main class="mx-auto max-w-6xl px-4 py-12">
	<section class="text-center">
		<h1 class="text-4xl font-extrabold text-(--color-site-primary) md:text-5xl">
			{officersPage.heading}
		</h1>
	</section>

	<section class="mt-10 grid gap-8">
		{#each officersPage.officers as officer}
			<article class="rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm">
				<h2 class="text-center text-3xl font-bold text-(--color-site-primary)">
					{officer.role}
				</h2>
